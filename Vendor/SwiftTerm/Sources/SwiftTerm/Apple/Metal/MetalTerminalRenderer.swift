...
final class MetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    private static func findMetallibURL() -> URL? {
        for bundle in candidateBundles() {
            ...
            if let resourceURL = bundle.resourceURL,
               let urls = try? FileManager().contentsOfDirectory(at: resourceURL,
    ...
    private static func candidateBundles() -> [Foundation.Bundle] {
        [
            Foundation.Bundle.main,
            Foundation.Bundle(for: MetalTerminalRenderer.self)
        ]
    }
}
#endif
