...
enum RemoteCurlCommandBuilder {
    ...
    static let redactionToken = "$APPLEPI_TOKEN"

    /// Health-check path appended to the configured base URL.
    static let healthzPath = "healthz"
    ...
    static func redacted(host: PiHostConfiguration) -> String? {
    ...
    /// usable base URL.
    static func full(host: PiHostConfiguration, token: String) -> String? {
        guard let url = healthzURL(for: host) else { return nil }
    ...
    static func containsPlaintextToken(_ command: String) -> Bool {
