...
pi-app is distributed as an open-source, ad-hoc signed macOS app unless the release notes say otherwise.
...
3. Unzip the app.
4. Move `pi-app.app` to `/Applications`.
5. Launch the app.
...
For local sessions, pi-app includes its own small Pi notification helper inside the app bundle. When notifications are enabled in the app settings, new local sessions are launched with that bundled helper. The app does not install packages into Pi and does not modify `~/.pi/agent/settings.json`.
...
Remote API mode can browse, start, and resume sessions on the remote host. pi-app does not delete remote session files, and local Finder/file actions are disabled or limited for remote paths.
...
Developers can build and install a permanent local app:

```sh
script/install_app.sh
```

This creates `dist/pi-app.app`, installs it to `/Applications/pi-app.app`, verifies the signature, and opens it.

For a build-only bundle:
